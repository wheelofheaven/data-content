...
brechenden Tongeschirrs.{{ footnote(id="2") }} In der Höhle standen
...
Krug um Krug voller Stimmen.

Achtzehn Monate bevor der Stein in die Höhle fuhr, war über einem
anderen Meer ein anderer Blitz aufgestiegen. Der
{% wiki(slug="rael") %}raëlianische{% end %} Kanon — das
...
markiert die Eröffnung der {% wiki(slug="apocalypse") %}Apokalypse{% end %},
des Zeitalters der *Enthüllung* im schlichten griechischen Wortsinn, und
die Bewegung zählt ihre Jahre von diesem Wendepunkt an, sodass 1946 das
Jahr eins ist.{{ footnote(id="1") }} Das Kapitel der ersten Botschaft,
das die Zählung erklärt, trägt genau diesen Titel —
{% libref(book="the-book-which-tells-the-truth", chapter=5, verse=1) %}„1946, Jahr 1 der neuen Ära"{% end %} —
und die eigene Auswahl des Boten ist an dasselbe Ereignis geheftet:
...
Das griechische *apokalypsis* bedeutet eine Aufdeckung — die Entfernung
...
{% wiki(slug="apocalypse") %}Lesart des Korpus{% end %}, dem Kanon
...
Zeitalters ereignisreich sind. Das waren sie.
...
Thomasevangeliums, seit dem vierten Jahrhundert vergraben.{{ footnote(id="7") }}
Binnen etwa eines Jahres gaben die Qumran-Krüge die ersten Rollen her.
Am 30. September 1946 wurde Claude Vorilhon, später
{% wiki(slug="rael") %}Raël{% end %}, geboren — „wenn du 1946 geboren
...
Begriff, der die {% wiki(slug="ufology") %}moderne Kontaktära{% end %}
...
Satz wiedergibt.{{ footnote(id="8") }} Am 14. Mai 1948 wurde der Staat
...
handschriften der Erde.

In jenem Herbst erreichte die andere Hälfte des Fundes — eine zweite
...
die die Rollen hielten, bemerkten es zur Zeit.

Der amerikanische Akt des Dramas begann im Februar 1948, als die Syrer
...
lassen, glich die Schrift mit dem Nash-Papyrus ab{{ footnote(id="3") }}
...
herziehen, und sie hörten nie damit auf.

## Verschiedenes zu verkaufen

Mar Samuel brachte seine vier Rollen im Januar 1949 nach Amerika und
...
unter der Überschrift „Verschiedenes zu verkaufen", brachte das *Wall
Street Journal* dies:
...
Der Käufer war ein New Yorker Bankier namens Sidney Esteridge, der
...
Yadin war nicht fertig. Seit den frühen 1960er Jahren hatte er mit Kando
...
Schatten einer Übertragung von Territorium.

## Der Skandal des Jahrhunderts

Die Höhlen waren nicht fertig. Beduinische Prospektoren — den
...
dokumentarischen Stil auf Kupfer graviert.

Höhle 4 war die Falle. Frank Moore Cross, der Jahre an den Sortiertischen
...
Der Skandal hatte Opfer mit Namen. John Allegro, der eine Agnostiker des
...
lautstärksten Widersacher sagen ließe."

Das vierzigjährige Embargo gebar die unvermeidliche Theorie: dass der
Vatikan auf den Rollen sitze, weil sie das Christentum widerlegten.
Michael Baigent und Richard Leighs Bestseller *The Dead Sea Scrolls
Deception* von 1991 verschaffte ihr Massenverbreitung. Hierüber ist die
...
haben, als wahr erweist."

Die Befreiung, als sie kam, kam von den signaturhaften Instrumenten des
...
nannte ihre Funktionäre in der *New York Times* „engstirnige Trottel",
...
eine Nacherzählung der {% wiki(slug="genesis") %}Genesis{% end %} nach
einem 364-Tage-Sonnenkalender — erscheint in etwa fünfzehn Abschriften
und wird im Damaskus-Dokument der Sekte selbst *als Autorität zitiert*.
...
das Herabkommen der *benei ha-Elohim* von Genesis 6, ihre Lehre, ihre
...
in Judäa, auf Aramäisch, war Henoch Schrift.

Der Text der kanonischen Bücher war ebenso unbeständig. Die Höhlen gaben
...
oder der Tora, waren zur Zeit Christi nicht endgültig festgelegt."

Zwei aus den Höhlen geborgene Lesarten sind für dieses Projekt wichtiger
als alle übrigen, und beide betreffen das Wort
{% wiki(slug="elohim") %}Elohim{% end %}.

Die erste ist eine einzige Zeile des Deuteronomium. Im überlieferten Text
sagt das Lied des Mose, der Höchste habe die Grenzen der Völker
festgelegt „nach der Zahl der Söhne *Israels*" — eine Wendung, die die
...
Zahl der Söhne der *Elohim*", jedes Volk einem der göttlichen Vielheit
zugewiesen, wobei {% wiki(slug="yahweh") %}Yahweh{% end %} Jakob als
seinen Anteil erhielt.{{ footnote(id="5") }} Die etablierte Textkritik,
keine Freundin der Schlussfolgerungen dieses Korpus, hält die
Qumran-Lesart für ursprünglich. Hier ist die
{% wiki(slug="plurality-of-gods") %}Vielheit{% end %}, aus der dieses
...
*himmlischer* Amtsträger, das Gericht vollzieht; und um die Behauptung zu
sichern, zitiert er {% libref(book="psalms", chapter=82, verse=1) %}Psalm 82,1{% end %}
— „Elohim nimmt Stellung in der Versammlung Els, inmitten der elohim hält
er Gericht" — und benennt die stehende *elohim*:
Melchisedek.{{ footnote(id="6") }} Eine Gemeinschaft toratreuer Judäer,
zwei Jahrhunderte vor den Rabbinern, las *elohim* als ein Wort, das ein
...
{% wiki(slug="list-of-etymological-readings") %}etymologische
Fall{% end %} des Korpus hatte nie einen besseren antiken Zeugen.

Die messianische Akte aus den Höhlen läuft in dieselbe Richtung. Die
...
Dreigespann, das {% wiki(slug="jesus") %}Jesus{% end %} den Boten des
...
Gemeingut Judäas im letzten Jahrhundert, bevor die Ära sich wendete.

## Kinder des Lichts, Kinder der Finsternis

Wer verbarg die Bibliothek? Die Mehrheitsantwort seit 1948 lauteten die
...
Wer immer sie soziologisch waren, theologisch waren sie eine
...
Hand Gottes. Ihre Pescher{{ footnote(id="4") }}-Kommentare lasen jede
...
seiner Diener, der Propheten, kundtat."

Sie irrten sich über den Zeitplan. Das Ende, das 68 n. Chr. kam, war eine
...
Apokalypse als *Enthüllung* — das Zeitalter, in dem die alten Texte als
Aufzeichnungen lesbar werden und dessen erforderliche Architektur keine
Wüstenfestung ist, sondern eine {% wiki(slug="embassy") %}Botschaft{% end %}
für eine {% wiki(slug="great-return") %}Rückkehr{% end %}
({% libref(book="the-book-which-tells-the-truth", chapter=7, verse=5) %}die Bomben, TBWTT 7:5–6{% end %},
...
*vieler* Gemeinschaften, in die Wildnis getragen, als der Krieg kam. So
...
vergruben, was sie nicht tragen konnten, und niemand kam zurück.

Die Felswände bewahrten auch die Papiere der nächsten Vernichtung, und
...
der Höhle der Briefe, Verträge enthielt, die auf *nabatäischem* Aramäisch
abgefasst waren, nach dem Recht des Karawanenkönigreichs
{% wiki(slug="petra") %}Petra{% end %}.{{ footnote(id="9") }} Die
...
{% wiki(slug="nabataeans") %}Nabatäer{% end %} sind für dieses Korpus
...
Kalender des Kanons, dieses.

## Zehn Minuten allein im Krug

1965 wurden die wiedervereinten Rollen auf dem Höhenzug in West-Jerusalem
...
die Söhne der Finsternis, gegossen in Beton und Stein.{{ footnote(id="10") }}
...
sie hätten die Vergangenheit berührt."

Ich kann über dieses Gefühl berichten, denn ich habe es unter
...
Szenenbildner von *Alien* arbeiteten im selben Register, wenngleich der
...
die andere, nach der Zählung des Kanons im Jahr 77 und laufend.

Ich schulde dem Leser noch eine Offenlegung, da dieses Korpus die Praxis
...
über die Generation des Kommentators als über die Habakuks.

## Zwei Uhren

Collins greift zu einem Volksmärchen, um seinen Gegenstand zu
...
Eine Adoleszenz, die im Jahr eins begann. Das ist die Gesamtheit der
...
*elohim* namens Melchisedek — in Krügen in einer Felswand, und die Krüge
...
- Der Eintrag {% wiki(slug="apocalypse") %}Apokalypse{% end %}, für die
  ...
  von Enthüllung und Katastrophe, auf die sich dieser Artikel stützt.
- Die Einträge {% wiki(slug="elohim") %}Elohim{% end %} und
  {% wiki(slug="plurality-of-gods") %}Götter-Vielheit{% end %}, für das
  Argument, für das 4QDeut(j) und 11QMelchizedek die
  Handschriftenzeugen sind.
...
- [*The Book Which Tells the Truth*](/library/the-book-which-tells-the-truth/),
  ...
  Ära, die durchgehend zitiert werden.
