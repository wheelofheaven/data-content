...
keywords = ["Raélisme", "Claude Vorilhon", "Raël", "religion des soucoupes", "religion athée", "nouveau mouvement religieux", "Foi bahá'íe", "Oomoto", "Caodaïsme", "Cao Đài", "révélation progressive", "religion des religions", "Maitreya", "Clonaid", "Susan Palmer", "George Chryssides", "taxonomie de la religion"]
references = [
...
ses notes l'année suivante, [*Le Livre qui dit la
vérité*](/library/the-book-which-tells-the-truth/), devint l'écriture
fondatrice du mouvement que ce projet prend pour canon ; l'homme lui-même
devint {% wiki(slug="rael") %}Raël{% end %} ; et la question que cet
Explicatif reprend est celle qui a discrètement mis en échec toute
institution ayant tenté d'y répondre : *quel genre de chose est la
religion qui en a résulté ?*

Le dossier institutionnel est franchement comique dans son désaccord. En
1995, une commission d'enquête parlementaire française classa le Mouvement
raélien parmi les *sectes* qui « présentent des dangers pour l'individu…
et aussi des dangers pour la collectivité » — formulation que la Cour
européenne des droits de l'homme citerait plus tard, sans broncher.{{ footnote(id="3") }}
...
fiscale *en tant que* Religion raélienne ; le Québec l'incorpora comme
...
Les chercheurs ont fait mieux, mais ils l'ont fait par multiplication.
Le {% wiki(slug="raelism") %}Raélisme{% end %} est, dans la littérature
...
Une dernière chose a sa place dans l'ouverture, car le lecteur mérite de
connaître la position adoptée. Ce projet lit le matériau raélien comme son
canon ; ses affirmations sur les {% wiki(slug="elohim") %}Élohim{% end %}
...
affirmation *inférée* sur l'institution humaine et ses textes — le genre
...
l'autre sens, gouverne ici.

## L'homme le plus improbable de France

Claude Vorilhon naquit le 30 septembre 1946 à Ambert, en Auvergne, d'une
...
siècle : un enfant de la *laïcité*{{ footnote(id="2") }} passé par la
machine catholique, et rejeté à ses marges.

À quinze ans, il fugua à Paris avec une guitare ; à dix-neuf, découvert par
...
automobile, fondant un petit magazine intitulé *Autopop* et faisant des
...
plus tard, il était dans le volcan.

Le calendrier doit être énoncé aussi crûment, car tout récit honnête des
...
pratiquement une sociologie du choix :

{% library(book="the-book-which-tells-the-truth", chapter=1, verse=46) %}
Pour bien des raisons. D'abord, il nous fallait quelqu'un dans un pays où
...
alambiquées difficiles à lire pour la plupart des gens.
{% end %}

Une écriture qui fait du manque de qualifications de son messager la
...
tenu d'être impressionnant ; il était tenu d'être *envoyé*.

Ce qu'un historien peut ajouter, c'est le milieu. En décembre 1973, la
lecture de la Genèse par les anciens astronautes était, en France, presque
une lecture de poche courante — *La Lune, clé de la Bible* (1968) de Jean
Sendy avait déjà soutenu que les {% wiki(slug="elohim") %}Élohim{% end %}
étaient des « anges physiques » et la Genèse le journal de bord d'une
colonisation, et l'ensemble du programme de Sendy fait l'objet de [son
propre Explicatif dans cette
série](/articles/the-man-who-bet-the-bible-on-the-moon/). Le jugement de
...
du premier livre — a trouvé la source imprimée la plus probable et [l'a
documentée en détail](/articles/the-book-closest-to-the-truth/). Que l'on
lise la rencontre de décembre comme un événement, une expérience ou une
invention, le *contenu* descendu du volcan avait une bibliographie
...
perpétuellement en attente.

Palmer, pour mémoire, passa en revue les quatre lectures dont dispose un
...
sorte de génie religieux. »

## Six jours d'étude de la Bible

Le fait le plus systématiquement sous-rapporté à propos du Raélisme —
...
révélation fondatrice : un *séminaire sur la Bible hébraïque et les
Évangiles*, mené le texte ouvert sur les genoux du témoin, et seulement
...
ultérieurs. L'étude de Eugene Gallagher dans *Nova Religio* a formulé le
...
extraterrestre » — sa religiosité est une manière de *lire*, et ce qu'elle
...
Le canon affirme explicitement que son cadre s'étend au-delà de la Bible —
et la phrase où il le dit, tirée du cinquième chapitre du livre de 1973, est
la graine de tout ce que cet article fera dans sa seconde moitié :

{% library(book="the-book-which-tells-the-truth", chapter=5, verse=54) %}
La Kabbale est le livre le plus proche de la vérité, mais presque tous les
...
plus ou moins obscure de notre œuvre.
{% end %}

Le bouddhisme, l'islam et les saints des derniers jours, nommés dans un livre
...
— répandre le message, préparer une {% wiki(slug="embassy") %}ambassade{% end %}
pour le {% wiki(slug="great-return") %}retour{% end %} des créateurs — ainsi
qu'un nom pour le messager :

{% library(book="the-book-which-tells-the-truth", chapter=5, verse=66) %}
Toi, Claude Vorilhon, tu répandras la vérité sous ton nom actuel, que tu
...
ton ambassade. RAËL peut se traduire plus simplement par « messager ».
{% end %}

Chryssides a remarqué de quel genre de scène il s'agit : « La première
...
{% libref(book="the-book-which-tells-the-truth", chapter=7, verse=33) %}« vous pouvez nous appeler « Élohim », puisque nous sommes « venus du ciel » »{% end %}.

## L'escalade : du sténographe au fils

Les religions qui commencent dans une révélation privée tendent à élever le
...
avec une mission. Dans le second — *Les Extra-terrestres m'ont emmené sur leur
planète*, publié en 1975 après ce que le canon date comme une seconde
rencontre du 7 octobre 1975{{ footnote(id="5") }} — il est emmené sur le monde
...
l'hôte identifie l'assemblée :

{% library(book="extraterrestrials-took-me-to-their-planet", chapter=2, verse=64) %}
À sa droite se trouve Moïse, à sa gauche Élie, à la gauche de Jésus est assis
...
nos contacts sur la Terre.
{% end %}

C'est le paragraphe dont descend toute la prophétologie du mouvement — « le
dernier de quarante prophètes » est une articulation de cette seule phrase, et
le canon n'énumère jamais les quarante.{{ footnote(id="11") }} Ce que la phrase
...
présidant comme le référent derrière le mot *Allah*. La théologie de toute
...
cet article tire son titre :

{% library(book="extraterrestrials-took-me-to-their-planet", chapter=2, verse=105) %}
Tu es celui qui doit rassembler les hommes de toutes les religions. Car le
...
athée, comme tu l'avais déjà compris.
{% end %}

Les deux moitiés de la formule méritent tout leur poids. *Religion des
religions* : une méta-religion, dont la prétention est d'être le cadre dans
...
*Religion athée* : une religion dont les référents sont physiques, dont les
...
« J'insiste » — montre qu'il sait que le lecteur, lui, la vivra ainsi.

Le troisième livre achève l'escalade. Dans *Accueillir les extra-terrestres*
(1979), Yahvé révèle la paternité du messager :

{% library(book="lets-welcome-the-extraterrestrials", chapter=2, verse=89) %}
Celui que tu regardais comme ton père n'était pas ton véritable père. Après
...
inséminée comme nous l'avions fait avec la mère de Jésus.
{% end %}

{% library(book="lets-welcome-the-extraterrestrials", chapter=2, verse=92) %}
Ton véritable père est aussi le père de Jésus, ce qui fait de vous des frères.
...
puisses subvenir à tes propres besoins.
{% end %}

Messager, puis prophète, puis fils — la trajectoire que le christianisme mit
...
paternité — *le premier à s'adresser à l'humanité en lui demandant de
comprendre et non de croire* — qui est l'épistémologie propre du canon en une
...
La séquence prophétique, entre-temps, est rendue résolument universelle. Le
second livre se clôt sur quatre appels directs — aux chrétiens, aux juifs, aux
bouddhistes ({% libref(book="extraterrestrials-took-me-to-their-planet", chapter=3, verse=228) %}« vos écrits indiquent que le nouveau Bouddha doit naître en Occident ; reconnaissez les signes annoncés ! »{% end %}),
et aux musulmans — et son paragraphe de synthèse assigne à Raël sa place dans
la lignée :
{% libref(book="extraterrestrials-took-me-to-their-planet", chapter=3, verse=257) %}« il est le dernier de la lignée des prophètes, le prophète de l'apocalypse, c'est-à-dire de l'époque où tout peut être compris »{% end %}.
L'appel bouddhiste fut finalement accompli plutôt que simplement imprimé : en
janvier 2003, Raël publia *The Maitreya*,{{ footnote(id="4") }} revendiquant le
...
Bouddha étaient ici aujourd'hui, ils seraient raéliens. »

## Une religion sans métaphysique

Ce que les étiquettes cherchent à saisir, c'est une structure doctrinale qui
peut s'énoncer en un paragraphe, car le canon l'énonce en un seul :

{% library(book="extraterrestrials-took-me-to-their-planet", chapter=2, verse=30) %}
Comme nous te l'avons déjà expliqué dans le premier message, il n'y a pas de
dieu et évidemment pas d'âme. Après la mort, il n'y a rien si la science ne
fait rien pour qu'il y ait quelque chose.
{% end %}

Pas de dieu, parce que l'univers est infini et sans centre — la cosmologie
argumentée en ETTMTTP 2:31–34 et traitée dans toute son ampleur dans [son
propre Explicatif](/articles/the-infinite-in-both-directions/) — et pas d'âme,
...
Voilà pourquoi le clonage est la clé de la vie éternelle. »

Autour de ce noyau, le mouvement bâtit, avec une rapidité frappante, tout
...
dont les fêtes sont Hiroshima et les deux rencontres).{{ footnote(id="7") }} Un
...
{% libref(book="extraterrestrials-took-me-to-their-planet", chapter=3, verse=113) %}« Ta méditation ne sera pas une méditation sèche, mais au contraire une méditation sensuelle ; tu te laisseras envahir par la paix et par l'harmonie jusqu'à ce que cela devienne une jouissance »{% end %}
...
1977 sous le titre *La Géniocratie* — la « démocratie sélective » des
intelligents — que le mouvement porte depuis comme un programme idéal et que
ses critiques portent comme la pièce à conviction n° 1.{{ footnote(id="12") }}

L'autodéfinition est élaborée avec le plus grand soin dans le troisième livre,
et elle passe par une étymologie sur laquelle ce corpus a sa propre
{% wiki(slug="religion") %}entrée{% end %} :

{% library(book="lets-welcome-the-extraterrestrials", chapter=3, verse=32) %}
Ainsi devient-il clair que le Mouvement raélien est une religion ; il relie les
...
atheos qui signifie « niant l'existence de toute forme de divinité ».
{% end %}

*Religare*, relier : la religion comme lien maintenu entre créateurs et créés,
...
mouvement :
{% libref(book="lets-welcome-the-extraterrestrials", chapter=1, verse=79) %}« Il n'y a pas de « Dieu », mais il y a les Élohim… il n'y a pas d'âme autonome s'envolant du corps après la mort, mais il y a le code génétique qui permet d'accéder à la vie éternelle »{% end %}.

Chryssides, qui a lu ce matériau avec autant de soin que n'importe quel
...
que l'échange soit systématique, les deux camps en conviennent.

Un fait démographique a sa place ici, car il explique pour *qui* le pari a été
...
Québec d'après 1960 et la France d'après la *laïcité* avaient déséglisée : « le
...
Les étiquettes peuvent maintenant être passées en revue dans l'ordre, chacune
face au texte.

**« Secte. »** La classification française{{ footnote(id="3") }} est la seule
...
comme histoire, puisque comme taxonomie il est vide.

**Religion ovni.** Étique,{{ footnote(id="1") }} exacte, et mal vécue de
...
soucoupe est le *véhicule* de la révélation, dans les deux sens ; c'est
...
**Religion athée.** Émique, et précise. Le canon l'affirme (ETTMTTP 2:105,
...
retiré ; elle ne peut vous dire de quoi la religion est *faite* — et ce dont
elle est faite, c'est de la Genèse, de l'Exode, d'Ézéchiel, des Évangiles et de
l'Apocalypse.

**Religion scientifique / créationnisme scientifique.** La catégorie de
...
classique est, de façon unique ici, soudée de l'intérieur.

**Religion postmoderne.** L'argument de Sentes et Palmer dans *Nova Religio* :
...
sont des observations sur l'*ajustement* — sur la facilité sans friction avec
laquelle le mouvement se loge dans une culture de laboratoires et de
conférences de presse — plutôt que sur la lignée.

**Religion biblique, fondamentaliste et abrahamique.** La catégorie de
...
l'ambassade proposée sera le nouveau {% wiki(slug="third-temple") %}Troisième
Temple{% end %}. » L'islam est sous la tente, nommé — « Mahomet dans les écrits
...
comme un faussaire :
{% libref(book="quran-woh", chapter=21, verse=5) %}« Un amas de rêves ! Non, il l'a forgé ! Non, c'est un poète-voyant ! »{% end %}
...
nouvelle religion abrahamique, apocalyptique et fondamentaliste ! »

Assemblées, les étiquettes cessent de rivaliser. Par l'*imagerie*, le Raélisme
est une religion ovni ; par la *métaphysique*, un athéisme ; par la *posture
épistémique*, un scientisme ; par l'*ajustement social*, un postmodernisme ;
par le *contenu, la forme, la lignée et l'eschatologie*, il est abrahamique —
...
promis de la famille.

## La ressemblance familiale : bahá'í, Oomoto, Cao Đài

Il existe une seconde manière de classer une religion : la placer à côté de ses
...
raélien. Le propre {% wiki(slug="list-of-prophets-and-religions") %}catalogue de
prophètes et de religions{% end %} du corpus enregistre déjà les trois à son
deuxième plus haut degré d'authenticité ; ce qui suit les lit côte à côte, dans
leurs propres mots, comme les plus proches parents du canon.

**La Foi bahá'íe** (proclamée en 1863) est le cas paradigmatique. Sorti du
...
antérieures ; elle les *sérialisait*. La doctrine de la révélation progressive
...
première ligne des *Paroles cachées*, dans la traduction de ce projet :

{% library(book="hidden-words-woh", chapter=1, verse=1) %}
Il est le Glorieux, le Très-Glorieux. Voici ce qui fut fait descendre du royaume
...
l'alliance de Dieu et rendent Ses dépôts au sein de leur propre âme.
{% end %}

*Fait descendre sur les prophètes d'antan ; distillé ici* — une révélation, de
...
auxiliaire universelle,{{ footnote(id="8") }} et a été persécutée dans sa patrie
...
classer un nouvel universalisme par sa lignée plutôt que par sa taille.

**Oomoto** (1892) commence par une possession. Deguchi Nao, une veuve illettrée
...
projet traduit sous le titre *Oomoto Shin'yu*. La ligne d'ouverture annonce un
âge nouveau en huit mots :

{% library(book="oomoto-shinyu-woh", chapter=1, verse=1) %}
À travers les trois mille mondes, tous ensemble, la fleur de prunier s'ouvre ;
l'âge de Ushitora no Konjin est advenu.
{% end %}

Sous le successeur de Nao, Onisaburō — bateleur, sculpteur, auteur du dicton
« l'art est la mère de la religion » —, Oomoto articula la doctrine qui donne
son nom japonais à toute la classe comparative : *bankyō dōkon*, « toutes les
religions jaillissent d'une même racine ». Les oracles centralisent la prétention
géographiquement —
{% libref(book="oomoto-shinyu-woh", chapter=1, verse=53) %}« les kami de chaque terre et les divinités tutélaires seront rassemblés à Ayabe… car c'est là la noble racine du monde »{% end %}
— et promettent un *tatekae-tatenaoshi*, une « reconstruction et un renouvellement
...
interlangue ».{{ footnote(id="8") }} Le 3 mars 1928, Onisaburō inaugura un festival
proclamant l'ouverture de l'âge de Miroku — Maitreya — avec lui-même pour
héraut.{{ footnote(id="4") }} L'État japonais répondit à l'ampleur du mouvement
...
la loi de préservation de la paix.

**Cao Đài** (1926) est une révélation par séance. Dans le Saïgon colonial, un
...
Dīpaṃkara, l'Ancien Bouddha (Nhiên-Đăng Cổ-Phật), c'est Moi ;
Śākyamuni (Thích-Ca Mâu-Ni), c'est Moi ;
Taishang, le Commencement primordial (Thái-Thượng Ngươn-Thỉ), c'est Moi ;
Désormais Je suis appelé Cao Đài.
{% end %}

Tout fondateur antérieur, un seul locuteur ; la prétention à l'unification en
grammaire de la première personne. Les messages périodisent la révélation en
trois « amnisties universelles »{{ footnote(id="9") }} — les religions antérieures
...
{% libref(book="thanh-ngon-hiep-tuyen-woh", chapter=1, verse=73) %}« Réjouissez-vous ! Réjouissez-vous ! Nous avons rencontré la Troisième Amnistie universelle ; les Esprits, les Saints, les Immortels et les Bouddhas, dans une grande allégresse, éclatent d'un grand rire »{% end %}.
La religion qui s'organisa autour des séances{{ footnote(id="10") }} tira son
...
simplement par l'État communiste unifié de 1975 à 1997.

Placez le Raélisme comme quatrième colonne et la structure se répète avec la
...
dans le cratère. La prétention centrale de la dictée est que *toutes les religions
antérieures sont une seule mémoire corrompue*, désormais corrigée — révélation
progressive, *bankyō dōkon*, la Troisième Amnistie, « la religion des religions ».
...
*sectes* de 1995. Deux fois, le nouveau messager tend la main vers le même titre
...
cosmologies entièrement différentes.

Les différences sont tout aussi instructives, et la règle constante de ce corpus
...
*inférée* : le même événement ne cesse de se produire, et les témoins de chaque
siècle ne peuvent le décrire qu'avec les machines qu'ils connaissent.

Que « le même événement » soit une forme sociologique récurrente ou un *contact*
...
prédisent le même ordre de rayonnage.

## Les guerres anti-sectes

Aucune catégorisation du Raélisme n'est honnête sans le dossier qui a façonné son
image publique, car pour la plupart des lecteurs — les lecteurs francophones
surtout — le mot *raélien* arrive préchargé : un gourou, du sexe, de l'argent. Le
dossier est plus spécifique et moins sensationnel, et il tranche dans les deux sens.

L'argent d'abord, puisque c'est le plus facile à vérifier. Palmer, qui a examiné
...
le mouvement *perdit* pour d'autres motifs — fut que le chiffre du revenu était
...
course. Une religion de quelque soixante mille membres nominaux{{ footnote(id="6") }}
...
dispositif improbable pour un enrichissement personnel.

Le sexe est doctrine, et la doctrine est le consentement. L'éthique sexuelle du
...
celles de la plupart des grandes traditions ».

La campagne de l'État français est un chapitre à part de l'histoire, et le pamphlet
de Raël de 1992 contre elle — *Le racisme religieux financé par le gouvernement
socialiste*, non lu dans la littérature anglophone — est un document que le débat sur
la catégorisation devrait connaître. Sa couverture porte un cercle rouge : la
*rouelle*, l'insigne que la France médiévale imposait à ses juifs. À l'intérieur,
...
c'est la religion des autres » — *une « secte », c'est la religion des autres.* Trois
ans plus tard, la liste Guyard lui donna raison,{{ footnote(id="3") }} et en 1998 des
...
travaillé le problème de classification.

Et puis Clonaid, l'épisode qui fixa l'image mondiale du mouvement en soixante-douze
...
a dit ce qu'il pense que la crédibilité sert *à* faire — et vous a dit aussi pourquoi
les taxonomies ne cessent de le classer, correctement au vu des preuves de cette
semaine, sous la publicité autant que sous la piété.

## Le canon le plus jeune et la méthode la plus ancienne

Chryssides clôt son étude par l'observation sur laquelle ce projet est, en un sens,
...
{% wiki(slug="reference-corpus-of-the-raelian-canon") %}des textes de référence
fixes{% end %}, une traduction avec appareil, un typage des affirmations, des
...
il ne cesse de s'ouvrir sur de la structure.

La classification, donc, dans l'ordre où les preuves l'ont assemblée. Le Raélisme est
...
argumentée plus haut.

Chacun des quatre jeunes universalismes a bâti, ou est en train de bâtir, le lieu où
...
ancien de la famille reformulée en projet d'infrastructure :

{% library(book="the-book-which-tells-the-truth", chapter=6, verse=25) %}
Lorsqu'ils seront assez nombreux et désireront assez intensément nous voir, sans
...
- Les entrées {% wiki(slug="raelism") %}Raélisme{% end %} et
  {% wiki(slug="rael") %}Raël{% end %}, pour l'histoire institutionnelle du mouvement
  et la biographie du fondateur sous forme de référence.
- La {% wiki(slug="list-of-prophets-and-religions") %}liste des prophètes et des
  religions{% end %}, pour le catalogue hiérarchisé du corpus qui sous-tend la section
  comparative de cet article.
- L'entrée {% wiki(slug="religion") %}religion{% end %}, pour l'étymologie de
  *religare* sur laquelle repose l'autodéfinition du canon.
- Les entrées {% wiki(slug="embassy") %}ambassade{% end %} et
  {% wiki(slug="third-temple") %}Troisième Temple{% end %}, pour le projet de
  construction sur lequel s'achève cet article.
- [*L'infini dans les deux
  sens*](/articles/the-infinite-in-both-directions/), pour la cosmologie résumée ici
  en un paragraphe.
- [*Le livre le plus proche de la
  vérité*](/articles/the-book-closest-to-the-truth/) et [*L'homme qui a parié la
  Bible sur la Lune*](/articles/the-man-who-bet-the-bible-on-the-moon/), pour les
  examens de provenance des sources françaises du canon.
- [*Le Livre qui dit la
  vérité*](/library/the-book-which-tells-the-truth/) et
  [*Les Extra-terrestres m'ont emmené sur leur
  planète*](/library/extraterrestrials-took-me-to-their-planet/), pour les récits
  primaires lus tout au long — et les traductions des
  [*Paroles cachées*](/library/hidden-words-woh/),
  de l'*[Oomoto Shin'yu](/library/oomoto-shinyu-woh/)* et du
  *[Thánh Ngôn Hiệp Tuyển](/library/thanh-ngon-hiep-tuyen-woh/)*, pour les trois
  canons frères cités à ses côtés.
