...
brise.{{ footnote(id="2") }} À l'intérieur de la grotte se dressaient des
...
Dix-huit mois avant que la pierre n'entre dans la grotte, un autre éclair
s'était élevé au-dessus d'une autre mer. Le canon
{% wiki(slug="rael") %}raélien{% end %} — le matériau source fondateur de
ce projet — date de lui l'âge présent : la destruction atomique de
Hiroshima, le 6 août 1945, marque l'ouverture de
l'{% wiki(slug="apocalypse") %}Apocalypse{% end %}, l'âge du *dévoilement*
au sens grec propre du mot, et le mouvement compte ses années depuis cette
charnière, de sorte que 1946 est l'an un.{{ footnote(id="1") }} Le
chapitre du premier message qui explique le compte porte exactement ce
titre —
{% libref(book="the-book-which-tells-the-truth", chapter=5, verse=1) %}« 1946, an 1 de la nouvelle ère »{% end %} —
et le choix du messager lui-même est arrimé au même événement :
...
Le grec *apokalypsis* signifie un découvrement — le retrait d'un voile, la
divulgation de ce qui était caché. Le sens catastrophique est une
accrétion médiévale ; la {% wiki(slug="apocalypse") %}lecture du
corpus{% end %}, à la suite du canon, prend le mot à son étymologie : l'âge
...
chargées. Elles le furent.

En décembre 1945, au pied des falaises du Jabal al-Tarif, en Haute-Égypte,
...
Thomas, enfouis depuis le quatrième siècle.{{ footnote(id="7") }} En
...
{% wiki(slug="rael") %}Raël{% end %}, naquit — « si tu es né en 1946, ce
...
l'{% wiki(slug="ufology") %}ère moderne du contact{% end %} ; l'incident de
...
restitue cette phrase.{{ footnote(id="8") }} Le 14 mai 1948, l'État
...
Cet automne-là, l'autre moitié de la trouvaille — un second rouleau
...
remarquèrent sur le moment.

L'acte américain du drame commença en février 1948, quand les Syriaques
...
Nash{{ footnote(id="3") }}, et expédia par avion des tirages à William
...
Mar Samuel emporta ses quatre manuscrits en Amérique en janvier 1949 et ne
...
« Divers à vendre », le *Wall Street Journal* publia ceci :

> **« Les quatre Manuscrits de la mer Morte. »**
...
> Boîte F 206, The Wall Street Journal.
>
> — *Wall Street Journal*, juin 1954, reproduit dans Collins, chap. 1

L'acheteur fut un banquier new-yorkais nommé Sidney Esteridge, qui paya
...
Sanctuaire du Livre, où cet article reviendra à pied.

Yadin n'en avait pas fini. Depuis le début des années 1960, il négociait
...
— chaque transfert de parchemin faisant ombre à un transfert de territoire.

## Le scandale du siècle

Les grottes n'en avaient pas fini. Des prospecteurs bédouins — toujours un
...
un style documentaire sec.

La Grotte 4 fut le piège. Frank Moore Cross, qui passa des années aux tables
...
Le scandale eut des victimes qui ont des noms. John Allegro, le seul
...
importuner », et fut anéanti en 1990 par un entretien à *Ha'aretz*, donné au
...
qu'on pourrait dire de certains de ses détracteurs les plus véhéments. »

L'embargo de quarante ans engendra la théorie inévitable : que le Vatican
gardait les manuscrits parce qu'ils réfutaient le christianisme. Le
best-seller de 1991 de Michael Baigent et Richard Leigh, *The Dead Sea
Scrolls Deception*, lui donna une circulation de masse. Sur ce point,
...
que ce que nous avons cru depuis toujours se révèle vrai. »

La libération, quand elle vint, vint des instruments emblématiques de l'âge
...
bornés » dans le *New York Times*, et le 27 octobre 1991 le monopole
...
la {% wiki(slug="genesis") %}Genèse{% end %} sur un calendrier solaire de
364 jours — figure en une quinzaine d'exemplaires et est *cité comme une
autorité* dans le propre Document de Damas de la secte. Collins expose sans
...
descente des *benei ha-Elohim* de la Genèse 6, leur enseignement, leurs
...
fixés au temps du Christ. »

Deux leçons récupérées des grottes importent plus à ce projet que tout le
reste, et toutes deux concernent le mot
{% wiki(slug="elohim") %}Élohim{% end %}.

La première est une seule ligne du Deutéronome. Dans le texte reçu, le
cantique de Moïse dit que le Très-Haut fixa les frontières des nations
« selon le nombre des fils d'*Israël* » — une formule qui a intrigué les
...
fils d'*Élohim* », chaque peuple attribué à l'un de la pluralité divine, avec
{% wiki(slug="yahweh") %}Yahvé{% end %} recevant Jacob pour sa
part.{{ footnote(id="5") }} La critique textuelle dominante, guère amie des
conclusions de ce corpus, juge la leçon de Qumrân originelle. Voici la
{% wiki(slug="plurality-of-gods") %}pluralité{% end %} dont ce projet
...
d'un Jour des Expiations final où Melchisédek, officier *céleste*, exécute le
jugement ; et pour asseoir la prétention, il cite le
{% libref(book="psalms", chapter=82, verse=1) %}Psaume 82:1{% end %} —
« Élohim se dresse dans l'assemblée d'El, au milieu des elohim il rend le
jugement » — et nomme l'*elohim* qui se dresse :
Melchisédek.{{ footnote(id="6") }} Une communauté de Judéens observant la
Torah, deux siècles avant les rabbins, lut *elohim* comme un mot qui pouvait
...
{% wiki(slug="list-of-etymological-readings") %}dossier étymologique{% end %}
du corpus n'a jamais eu de meilleur témoin antique.

Le dossier messianique issu des grottes court dans le même sens. La Règle de
...
{% wiki(slug="jesus") %}Jésus{% end %} récite aux messagers du Baptiste en
...
avant que l'ère ne tournât.

## Enfants de lumière, enfants de ténèbres

Qui cacha la bibliothèque ? La réponse majoritaire depuis 1948 est : les
...
Qui qu'ils fussent sociologiquement, théologiquement ils étaient une
...
pesher{{ footnote(id="4") }} lisaient chaque ancienne prophétie comme des
...
connaître tous les mystères des paroles de ses serviteurs les Prophètes. »

Ils se trompaient sur le calendrier. La fin qui vint en 68 apr. J.-C. fut une
...
l'apocalypse comme un *dévoilement* — l'âge où les anciens textes deviennent
lisibles comme des archives, et dont l'architecture requise n'est pas un
réduit désertique mais une {% wiki(slug="embassy") %}ambassade{% end %} pour
un {% wiki(slug="great-return") %}retour{% end %}
({% libref(book="the-book-which-tells-the-truth", chapter=7, verse=5) %}les bombes, LLQDV 7:5–6{% end %},
...
de *plusieurs* communautés, portées au désert quand la guerre survint. Dans
...
Les falaises gardèrent aussi les papiers de l'annihilation suivante, et ici
...
Grotte des Lettres, comprenait des contrats rédigés en araméen *nabatéen*,
sous la loi du royaume caravanier de {% wiki(slug="petra") %}Pétra{% end %}.{{ footnote(id="9") }}
...
les deux corpus. Les {% wiki(slug="nabataeans") %}Nabatéens{% end %} importent
...
la lecture, sur le calendrier du canon, c'est celui-ci.

## Dix minutes seul dans la jarre

En 1965, les manuscrits réunis furent installés sur la crête de
...
béton et la pierre.{{ footnote(id="10") }} On entre par un passage bas
...
avec le sentiment d'avoir touché le passé. »

Je puis témoigner de ce sentiment, car je l'ai éprouvé dans des conditions
...
que de cette planète ; les chefs décorateurs d'*Alien* travaillaient dans le
...
Je dois au lecteur une dernière divulgation, puisque ce corpus a pour
...
Collins recourt à un conte populaire pour décrire son sujet : la biographie
...
biographie du corpus n'en est encore qu'à son adolescence. »

Une adolescence qui commença en l'an un. C'est là toute la prétention
...
Hénoch, sa cosmologie à deux esprits, ses « fils d'Élohim », son *elohim*
...
- L'entrée {% wiki(slug="apocalypse") %}Apocalypse{% end %}, pour le
  ...
  dévoilement et catastrophe sur laquelle cet article s'appuie.
- Les entrées {% wiki(slug="elohim") %}Élohim{% end %} et
  {% wiki(slug="plurality-of-gods") %}pluralité des dieux{% end %}, pour
