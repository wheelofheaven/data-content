...
rompiéndose.{{ footnote(id="2") }} Dentro de la cueva se alzaban hileras
...
Dieciocho meses antes de que la piedra entrara en la cueva, un destello
distinto había ascendido sobre un mar distinto. El canon
{% wiki(slug="rael") %}raeliano{% end %} —el material fundacional de
...
apertura del {% wiki(slug="apocalypse") %}Apocalipsis{% end %}, la era
del *develamiento* en el sentido llano y griego de la palabra, y el
movimiento cuenta sus años desde esa bisagra, de modo que 1946 es el año
uno.{{ footnote(id="1") }} El capítulo del primer mensaje que explica el
cómputo se titula exactamente así —
{% libref(book="the-book-which-tells-the-truth", chapter=5, verse=1) %}«1946, año 1 de la nueva era»{% end %}—
y la propia elección del mensajero está fijada al mismo suceso:
...
debajo son comprobables, y la comprobación es el placer del asunto.

## Un calendario que arranca con un destello

El griego *apokalypsis* significa un descubrimiento —la retirada de un
velo, la revelación de lo que estaba oculto—. El sentido catastrófico es
una acreción medieval; la {% wiki(slug="apocalypse") %}lectura del
corpus{% end %}, siguiendo al canon, toma la palabra en su etimología:
...
ajetreados. Lo fueron.

En diciembre de 1945, al pie de los riscos del Jabal al-Tarif, en el
...
IV—.{{ footnote(id="7") }} Al cabo de aproximadamente un año, las
tinajas de Qumrán entregaron los primeros rollos. El 30 de septiembre
de 1946 nació Claude Vorilhon, más tarde {% wiki(slug="rael") %}Raël{% end %}
...
término que dio nombre a la {% wiki(slug="ufology") %}era de contacto
moderna{% end %}; el incidente de Roswell siguió en un par de semanas. El
...
la información, incluido el que compone esta frase—.{{ footnote(id="8") }}
...
El acto americano del drama comenzó en febrero de 1948, cuando los
...
Papiro Nash{{ footnote(id="3") }} y envió por correo aéreo las copias a
...
Mar Samuel se llevó sus cuatro rollos a América en enero de 1949 y no
...
de 1954, bajo el epígrafe «Varios en venta», el *Wall Street Journal*
publicó esto:
...
>
> — *Wall Street Journal*, junio de 1954, tal como se reproduce en
> Collins, cap. 1

El comprador fue un banquero neoyorquino llamado Sidney Esteridge, que
...
que este artículo volverá a pie.

Yadin no había terminado. Desde principios de los años sesenta venía
...
—cada traspaso de pergamino como sombra de un traspaso de territorio—.

## El escándalo del siglo

Las cuevas no habían terminado. Los prospectores beduinos —siempre un
...
La Cueva 4 fue la trampa. Frank Moore Cross, que pasó años en las mesas
...
en el escándalo académico por excelencia del siglo XX».

El escándalo tuvo víctimas con nombre. John Allegro, el único agnóstico
...
que podría decirse de algunos de sus más ruidosos detractores».

El embargo de cuarenta años engendró la teoría inevitable: que el
...
1991, *The Dead Sea Scrolls Deception*, le dio circulación masiva. En
...
lo que creímos todo el tiempo resulta ser cierto».

La liberación, cuando llegó, vino de los instrumentos característicos de
...
William Safire llamó a sus funcionarios «zopencos insulares» en el *New
York Times*, y el 27 de octubre de 1991 el monopolio se derrumbó. Bajo
...
Todos los libros de la {% wiki(slug="hebrew-bible") %}Biblia
Hebrea{% end %} salvo Ester aparecieron en las cuevas —y también, en
cantidad, libros que el canon posterior expulsó—. Fragmentos del
[Libro de Enoc](/library/book-of-enoch/) afloraron en su arameo
...
{% wiki(slug="genesis") %}Génesis{% end %} sobre un calendario solar de
364 días— aparece en unas quince copias y es *citado como autoridad* en
...
material de los Vigilantes de Enoc —el descenso de los *benei ha-Elohim*
...
C., en Judea, en arameo, Enoc era Escritura.

El texto de los libros canónicos estaba igualmente inestable. Las cuevas
...
Dos lecturas recuperadas de las cuevas importan a este proyecto más que
todas las demás, y ambas conciernen a la palabra
{% wiki(slug="elohim") %}Elohim{% end %}.

La primera es una sola línea del Deuteronomio. En el texto recibido, el
cántico de Moisés dice que el Altísimo fijó las fronteras de las
naciones «según el número de los hijos de *Israel*» —una expresión que
...
se dividieron «según el número de los hijos de *Elohim*», cada pueblo
asignado a uno de la pluralidad divina, recibiendo
{% wiki(slug="yahweh") %}Yahvé{% end %} a Jacob como su
porción.{{ footnote(id="5") }} La crítica textual dominante, nada amiga
de las conclusiones de este corpus, juzga original la lectura de Qumrán.
Aquí está la {% wiki(slug="plurality-of-gods") %}pluralidad{% end %} de
...
el que Melquisedec, un oficial *celestial*, ejecuta el juicio; y para
asegurar la afirmación cita el
{% libref(book="psalms", chapter=82, verse=1) %}Salmo 82:1{% end %} —«Elohim
se yergue en la asamblea de El, en medio de los elohim imparte juicio»—
y nombra al *elohim* que está en pie: Melquisedec.{{ footnote(id="6") }}
Una comunidad de judeos observantes de la Torá, dos siglos antes de los
rabinos, leyó *elohim* como una palabra que podía designar a un miembro
de una clase de seres poderosos y la aplicó a un individuo que no era ni
el Altísimo ni una metáfora. El {% wiki(slug="list-of-etymological-readings") %}caso
etimológico{% end %} del corpus nunca ha tenido mejor testigo antiguo.

El expediente mesiánico de las cuevas discurre en la misma dirección. La
...
no isaiano que {% wiki(slug="jesus") %}Jesús{% end %} recita a los
...
Judea en el último siglo antes de que la era cambiara.

## Hijos de la luz, hijos de las tinieblas

¿Quién escondió la biblioteca? La respuesta mayoritaria desde 1948 han
...
hombres de Qumrán con sus perennes huéspedes, los esenios».

Fueran quienes fueran sociológicamente, teológicamente eran una
...
pesher{{ footnote(id="4") }} leían toda profecía antigua como despachos
...
Se equivocaron en el calendario. El fin que llegó en el año 68 d. C. fue
...
lee el apocalipsis como *develamiento* —la era en que los textos antiguos
se vuelven legibles como registros, y cuya arquitectura requerida no es
un reducto en el desierto sino una {% wiki(slug="embassy") %}embajada{% end %}
para un {% wiki(slug="great-return") %}retorno{% end %}
({% libref(book="the-book-which-tells-the-truth", chapter=7, verse=5) %}las bombas, TBWTT 7:5–6{% end %},
...
distancia moral que a este corpus le importa.
...
redactados en arameo *nabateo*, bajo la ley del reino caravanero de
{% wiki(slug="petra") %}Petra{% end %}.{{ footnote(id="9") }} La conexión
...
los dos corpus. Los {% wiki(slug="nabataeans") %}nabateos{% end %}
...
de la lectura, en el calendario del canon, es esta.

## Diez minutos a solas en la tinaja

En 1965 los rollos reunidos se instalaron sobre la loma, en el oeste de
...
vertidos en hormigón y piedra—.{{ footnote(id="10") }} Se entra por un
...
De esa sensación puedo dar testimonio, porque la he puesto a prueba en
...
*Alien* trabajaron en el mismo registro, aunque el Santuario lo lleva con
...
canon, va por el año 77 y sigue corriendo—.

Le debo al lector una revelación más, ya que este corpus practica mostrar
...
generación del comentarista que sobre la de Habacuc—.

## Dos relojes

Collins echa mano de un cuento popular para describir su asunto: la
...
del día. La biografía del corpus está aún en su adolescencia».

Una adolescencia que empezó en el año uno. Eso es la totalidad de la
...
Elohim», su *elohim* celestial llamado Melquisedec— en tinajas dentro de
...
despiertas en el año uno, aún leídas en el año 81—.
...
- La entrada {% wiki(slug="apocalypse") %}Apocalipsis{% end %}, para el
  ...
  develamiento y catástrofe en la que este artículo se apoya.
- Las entradas {% wiki(slug="elohim") %}Elohim{% end %} y
  {% wiki(slug="plurality-of-gods") %}pluralidad de dioses{% end %},
...
- [*El Libro que dice la Verdad*](/library/the-book-which-tells-the-truth/),
  ...
  era citados a lo largo del texto.
